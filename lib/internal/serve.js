...
class WrapperHandler {
  ...
  handleRequest(url) {
    ...
    try {
      ...
      const query = search;
      const meta = [...this.getMeta(url)].join("\n");
      const script = [...this.getScript(url)].join("\n");
   ...
   * associated js file.
   *
   * @param url The URL of the request being processed.
   * @returns {Iterable<[string, string]>}
   */
  * getMetadata(url) {
    const path = this.getPath(filesystemPath(this.basePath, url, this.urlBase), false);
   ...
   */
  * getMeta(url) {
    for (const [key, value] of this.getMetadata(url)) {
  ...
  * getScript(url) {
   ...
   * @returns {void}
   */
  checkExposure(url) { // eslint-disable-line no-unused-vars
    // do nothing
  }
}
