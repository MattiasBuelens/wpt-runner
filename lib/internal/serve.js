...
class WorkersHandler extends HtmlWrapperHandler {
  pathReplace() {
    return [
      [".any.worker.html", ".any.js", ".any.worker.js"],
      [".worker.html", ".worker.js"]
    ];
  }

  wrapper(meta, script, path, query) {
    return `<!doctype html>
<meta charset=utf-8>
${meta}
...
<div id=log></div>
<script>
fetch_tests_from_worker(new Worker("${path}${query}"));
</script>
`;
  }
}
...
class SharedWorkersHandler extends HtmlWrapperHandler {
  pathReplace() {
    return [
      [".any.sharedworker.html", ".any.js", ".any.worker.js"]
    ];
  }
...
<div id=log></div>
<script>
fetch_tests_from_worker(new SharedWorker("${path}${query}"));
</script>
`;
  }
}

class ServiceWorkersHandler extends HtmlWrapperHandler {
  pathReplace() {
    return [
      [".any.serviceworker.html", ".any.js", ".any.worker.js"]
    ];
  }
...
(async function() {
  ...
  reg = await navigator.serviceWorker.register("${path}${query}", {scope});
...
class AnyWorkerHandler extends WrapperHandler {
  headers() {
...
`;
  ...
  scriptReplacement(key, value) {
    if (key === "script") {
      const attribute = value.replace("\\", "\\\\").replace("\"", "\\\"");
      return `importScripts("${attribute}")`;
    }
    if (key === "title") {
      value = value.replace("\\", "\\\\").replace("\"", "\\\"");
...
exports.WorkersHandler = WorkersHandler;
exports.WindowHandler = WindowHandler;
exports.AnyHtmlHandler = AnyHtmlHandler;
exports.SharedWorkersHandler = SharedWorkersHandler;
exports.ServiceWorkersHandler = ServiceWorkersHandler;
exports.AnyWorkerHandler = AnyWorkerHandler;
