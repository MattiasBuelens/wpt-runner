...
const { ResourceLoader } = require("./internal/resource-loader.js");
...
module.exports = (testsPath, {
  ...
  const host = "http://web-platform.test";
  const urlPrefix = `${host}${rootURL}`;
  const resourceLoader = new ResourceLoader(host, testsPath, rootURL);
    ...
    function doTest(index) {
      ...
      return Promise.resolve(filter(testPath, url)).then(result => {
        if (result) {
          ...
          return runTest(url, setup, resourceLoader, reporter).then(success => {
            if (!success) {
              ++totalFailures;
            }
          });
...
function runTest(url, setup, resourceLoader, reporter) {
